...
class ImmoFetch(threading.Thread):
    ...
    our_table_annonces = 'immoanno'
    our_table_photos   = 'immoimg'
    ...
    def init_static(ops_pointeur):
        ...
        if len(ImmoFetch.our_ville2codep) == 0 :
            liste_obj = list(r.table('ress_villesfr').filter(lambda row : r.expr(['38','73']).contains(row['departement'])).pluck('codepostal', 'nom_stz').run(ImmoFetch.our_ops.rdb))
    ...
    def push_images_to_rdb(obj):
        ...
        for url_image in obj['url_images'] :
            ...
            try :
                ...
                reponse = r.table(ImmoFetch.our_table_photos).insert(obj_image, conflict="replace").run(ImmoFetch.our_ops.rdb_get_lock()) # conflict="update" ou replace ou error
                ImmoFetch.our_ops.rdb_release()
                if reponse['inserted'] == 1 or reponse['replaced'] == 1:
    ...
    def push_item_to_rdb(anno):
        ...
        if tmpAnnonceInDB is None :
            ...
            reponse = r.table(ImmoFetch.our_table_annonces).insert(obj, conflict="error").run(ImmoFetch.our_ops.rdb_get_lock()) # conflict="update" ou replace
            ImmoFetch.our_ops.rdb_release()
        ...
        # ---- Deja dans la DB, Update necessaire ?
            ...
            try :
                r.table(ImmoFetch.our_table_annonces).get(obj['id']).replace(tmpAnnonceInDB).run(ImmoFetch.our_ops.rdb_get_lock())
                ImmoFetch.our_ops.rdb_release()
            except Exception as e :
    ...
    def run(self):
        ...
        # -- Init
        liste_urls_queries  = ImmoFetch.our_ops.cfg.get(self.my_name) or []
        liste_urls_annonces = []
        liste_items_for_rdb = []
        liste_villes_exclure = ImmoFetch.our_ops.cfg.get('localites_exclure') or []
        ...
        nb_anno_anomaly = nb_anno_listed - nb_anno_discarded - nb_anno_parsed_ok
        logging.info("Site %s (%d sec) : %s listed | %d discarded | %d anomalies | %d parsed OK | %d new inserted | %d updated in db" % (self.my_name, int(t1-t0), nb_anno_listed, nb_anno_discarded, nb_anno_anomaly, nb_anno_parsed_ok, nb_anno_inserted_in_db, nb_anno_updated_in_db))

        self.our_ops.insertKPI(measurement='number', value=int(nb_anno_anomaly), tags={'component' : 'python', 'payload' : 'yes', 'number_of' : 'annonces_anomaly'})
        self.our_ops.insertKPI(measurement='number', value=int(nb_anno_discarded), tags={'component' : 'python', 'payload' : 'yes', 'number_of' : 'annonces_discarded'})
        self.our_ops.insertKPI(measurement='number', value=int(nb_anno_parsed_ok), tags={'component' : 'python', 'payload' : 'yes', 'number_of' : 'annonces_checked'})
        self.our_ops.insertKPI(measurement='number', value=int(nb_anno_updated_in_db), tags={'component' : 'python', 'payload' : 'yes', 'number_of' : 'annonces_updated'})
        self.our_ops.insertKPI(measurement='number', value=int(nb_anno_inserted_in_db), tags={'component' : 'python', 'payload' : 'yes', 'number_of' : 'annonces_new'})
...
if __name__ == '__main__':
    ...
    logging.basicConfig(level=logging.INFO-2, datefmt="%m-%d %H:%M:%S", format="P%(process)d|T%(thread)d|%(levelname)s|%(asctime)s | %(message)s")  # %(thread)d %(funcName)s L%(lineno)d
    ...
    ops = Ops4app.get_instance(appli_uname='fetcher.immo', rdb_ip_override='192.168.75.97', appli_cfg_override={'splashserver' : '192.168.75.97'})  # TODO : enlever les override de l'IP
    nblaunch = 0
    if ops :
